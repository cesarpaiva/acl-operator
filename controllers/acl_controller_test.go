...
func (suite *ControllerSuite) TestACLReconcilerKubernetesSelectorReconcile() {
	ctx := context.Background()
	acl := &v1alpha1.ACL{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "gitops-app",
			Namespace: "workspace-squad-a",
		},
		Spec: v1alpha1.ACLSpec{
			Source: v1alpha1.ACLSpecSource{
...
func (suite *ControllerSuite) TestACLReconcilerKubernetesSelectorEmptySourceMatchLabels() {
	ctx := context.Background()
	acl := &v1alpha1.ACL{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "empty-source",
	...
	suite.Assert().Equal("No podSelector generated by spec.source", existingACL.Status.Reason)
	suite.Assert().Empty(existingACL.Status.NetworkPolicy)
}

func (suite *ControllerSuite) TestACLReconcilerKubernetesSelectorEmptyDestinationMatchLabels() {
	ctx := context.Background()
	acl := &v1alpha1.ACL{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "empty-dest",
	...
	suite.Require().NoError(err)
	suite.Assert().False(existingACL.Status.Ready)
	suite.Assert().Contains(existingACL.Status.Reason, "could not generate egress rule for destination")
}
